...
tophat genes.gtf -g 1 --no-novel-indels --no-coverage-search --transcriptome-index=mm10/transcriptome_data --read-mismatches 3 --read-edit-dist 3 --read-realign-edit-dist 0 -a 12 -m 1 -o output_directory mm10/Bowtie2Index/genome input_file_1 input_file_2
...
#Blat Correction analysis was performed using the original script provided from the REDItools website (http://reditools.sourceforge.net):
...
ANNOVAR/table_annovar.pl converted_file annovar_files/mousedb/mm10 -buildver mm10 -remove -protocol refGene,snp142 -operation g,f -nastring . -csvout -out annotated_file
